<?php
/* This script includes a function which takes as input a) the existing cart line item index, and b) the relevant item_id.
...
function compute_other_line_item_quantity_total($item_id, $existing_cart_line_item_indx) {
    ...
    foreach ($_SESSION["cart"] as $cart_line_item_indx => $cart_line_item) {
        ...
        if ($cart_line_item["item_id"] == $item_id
            && $cart_line_item_indx != $existing_cart_line_item_indx) {  // if the item id matches, but the line item is not the same one being compared
